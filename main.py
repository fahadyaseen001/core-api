...
# Helper function to convert values for database compatibility
def convert_value_for_db(key: str, value: Any) -> Any:
    ...
    # Integer fields that might come as floats
    integer_fields = [
...
async def fetch_weather_data(lat: float, lon: float, forecast_days: int = 7, timezone: str = "auto") -> Dict[str, Any]:
    ...
    # Construct the base URL for Open-Meteo API
...
async def store_weather_data(data: Dict[str, Any], supabase: Client) -> Dict[str, Any]:
    ...
    try:
        # Sanitize all data for database compatibility
        ...
        if sanitized_data["current"]:
            try:
                ...
                # Continue with other data even if current fails

        # Store hourly forecasts in batches to avoid payload size limits
...
async def predict_outage_probability_rule_based(coords: LocationCoordinates):
    ...
    # --- Rule 3: Moderate Wind + Precipitation ---
    ...
    # --- Rule 5: Moderate Network Degradation + Weather ---
...
async def get_weather(
    ...
    try:
        ...
        # Store network data if provided
        if any([coords.effective_type, coords.downlink, coords.rtt]):
            ...
            try:
                ...
                # Continue even if network data storage fails
